...
class CloudPrintProxy(object):
        ...
        if not self.auth:
            ...
            try:
                ...
                xmpp_response = r.post(
                    ...
                    'application/x-www-form-urlencoded')
                jid = self.username if '@' in self.username else \
                    self.username + '@gmail.com'
                sasl_token = (
                    '\0%s\0%s' % (jid, xmpp_response['Auth'])).encode('base64')
    ...
    def get_rest(self):
        ...
        auth = self.get_auth()
        return check_new_auth(rest.REST(
            'https://www.google.com', auth=auth, debug=False))
...
def match_re(prn, regexps, empty=False):
    if len(regexps):
        try:
            return re.match(
                regexps[0], prn, re.UNICODE) or match_re(prn, regexps[1:])
        except:
            sys.stderr.write(
                'cloudprint: invalid regular expression: ' + regexps[0] + '\n')
            sys.exit(1)
    else:
        return empty
...
def sync_printers(cups_connection, cpp):
    ...
    local_printer_names = set([
        prn for prn in local_printer_names if match_re(prn, cpp.include, True)
    ])
    local_printer_names = set([
        prn for prn in local_printer_names if not match_re(prn, cpp.exclude)])
    ...
    for printer_name in local_printer_names - remote_printer_names:
        try:
            ...
            description = cups_connection.getPrinterAttributes(
                printer_name)['printer-info']
...
def process_job(cups_connection, cpp, printer, job):
    ...
    try:
        ...
        cups_connection.printFile(
            printer.name, tmp.name, job['title'], options)
        os.unlink(tmp.name)
    ...
    while True:
        try:
            ...
            if not xmpp_conn.is_connected():
                xmpp_conn.connect(XMPP_SERVER_HOST,
                                  XMPP_SERVER_PORT,
                                  XMPP_USE_SSL,
                                  xmpp_auth)
            ...
            LOGGER.error(
                ...
                'Try again in %d Seconds' % FAIL_RETRY)
...
def main():
    ...
    parser.add_argument(
        '-l', dest='logout', action='store_true',
        help='logout of the google account')
    parser.add_argument(
        '-p', metavar='pid_file', dest='pidfile', default='cloudprint.pid',
        help='path to write the pid to (default %(default)s)')
    parser.add_argument(
        '-a', metavar='account_file', dest='authfile',
        default=os.path.expanduser('~/.cloudprintauth'),
        help='path to google account ident data (default %(default)s)')
    parser.add_argument(
        '-c', dest='authonly', action='store_true',
        help='establish and store login credentials, then exit')
    parser.add_argument(
        '-f', dest='fastpoll', action='store_true',
        help='use fast poll if notifications are not working')
    parser.add_argument(
        '-u', dest='storepw', action='store_true',
        help='store username/password in addition to login token')
    parser.add_argument(
        '-i', metavar='regexp', dest='include', default=[], action='append',
        help='include local printers matching %(metavar)s')
    parser.add_argument(
        '-x', metavar='regexp', dest='exclude', default=[], action='append',
    ...
    if args.daemon:
        try:
            ...
            print '\tyum install python-daemon, or apt-get install ' \
